...
exports.findAndJoinTableFetch = void 0;
const findAndJoinTableFetch = async (options, needsPagination, dbconnection) => {
    ...
    for (let Joiner of options.foreignKeys) {
        query = query.join(Joiner.joinTable, `${primaryTble}.${Joiner.foreignKey}`, "=", `${Joiner.joinTable}.${Joiner.localKey}`);
    }
    ...
    if (needsPagination) {
        ...
        const result = await query.orderBy(options.orderBy.col, options.orderBy.order);
        return { result };
    }
};
exports.findAndJoinTableFetch = findAndJoinTableFetch;
